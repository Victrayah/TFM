...
tTable <- kable(resultsTable, format = "html", escape = F) %>%
  ...
  pack_rows("Sedation_level_final_day1 binarized (%|x, CI 95%)", 34,36) %>%
  pack_rows("benzo_day1 binarized (%|x, CI 95%)", 37,38) %>%
  pack_rows("Neurological status (%|x, CI 95%)", 39,51) %>%
  pack_rows("Ventilatory support (%|x, CI 95%)", 52,55) %>%
  pack_rows("Survival (%|x, CI 95%)", 56,61) %>%
  pack_rows("Length of stay (%|x, CI 95%)", 62,64 ) %>%
  pack_rows("Status_28_f binarized (%|x, CI 95%)", 65,70 )
